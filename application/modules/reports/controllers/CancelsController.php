<?php
class reports_CancelsController extends My_Controller_Action
{
	protected $_clase = 'rcancelaciones';
    ...
    public function indexAction(){
    	try{
			...
			$cViajes 		= new My_Model_RepViajes();
			...
			$aResume 	= $cViajes->getTravelsResumeCancels($aFilters);
			$totalResume= $this->countall($aResume);
			$aTravels	= $cViajes->getDetailsCancel($aFilters);
    ...
    public function countall($items){
    	$totalRows = 0;
    	foreach($items as $key => $subitems){
    		$totalRows += $subitems['TOTAL'];
    	}
    	return $totalRows;
    }
