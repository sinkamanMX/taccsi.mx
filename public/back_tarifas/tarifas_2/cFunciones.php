<?php
	...
	function getZona($iLatitud,$iLongitud,$tipoTaxi){
	    ...
	    $sql ="SELECT Z.ID_ZONA, Z.TIPO_ZONA, Z.DESCRIPCION AS N_ZONA, Z.COSTO, Z.COSTO_ACUMULABLE,Z.LATITUD, Z.LONGITUD,Z.RADIO, 
				...
				T.* , IF(CURRENT_TIME BETWEEN T.HORARIO_INICIO  AND T.HORARIO_FIN, T.COSTO_RECORRIDO, T.COSTO_FUERA_HORARIO ) AS N_COSTO, T.ID_CLASE AS N_CLASE				
			FROM ADMIN_ZONAS Z 
	...
	function getZonaInit($iLatitud,$iLongitud,$tipoTaxi){
	    ...
	    $sql ="SELECT Z.* ,T.* ,
				...
				IF(CURRENT_TIME BETWEEN T.HORARIO_INICIO  AND T.HORARIO_FIN, Z.COSTO, T.COSTO_FUERA_HORARIO ) AS COSTO_NT ,T.ID_CLASE AS N_CLASE
				FROM ADMIN_ZONAS Z
				INNER JOIN ADMIN_TARIFAS T ON Z.ID_TARIFA = T.ID_TARIFA
				WHERE Z.ID_TARIFA 
				IN (
