...
function calcAndDraw(inputLatitud,inputLongitud,iStatus){
	var latitud = '';
	var longitud= '';

	if(iStatus==2){
		latitud  = $("#inputLatOrigen").val();
		longitud = $("#inputLonOrigen").val();
	}else if(iStatus==5){
		latitud  = $("#inputLatDestino").val();
		longitud = $("#inputLonDestino").val();
	}

	if(latitud !="" && longitud !="" && inputLatitud !="" && inputLongitud!=""){		
		var latsOrigen  = new google.maps.LatLng(latitud, longitud);
	        ...
	        success: function(datos){	
				...
				if(result!= ""){
					...
					if(iStatus==5 || iStatus==2){
						...
						if(iStatus==2){
							$("#spanLeyendTime").html("Tiempo Estimado de arribo a Origen");
						}else{
							$("#spanLeyendTime").html("Tiempo Estimado de arribo a Destino");
						}	

						calcAndDraw(latTaxi,lonTaxi,iStatus);
