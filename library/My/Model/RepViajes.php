 ...
 * @author EPENA
 */
class My_Model_RepViajes extends My_Db_Table
{
    ...
    */
    ...
    public function getTravelsResumeCancels($aFilter){
		$result= Array();		
		$sql  = "SELECT IF(R.ID_RAZON_CANCELACION IS NULL,'-1',R.ID_RAZON_CANCELACION) AS ID,COUNT(V.ID_VIAJES) AS TOTAL, IF(R.DESCRIPCION IS NULL,'Sin Razon',R.DESCRIPCION) AS N_CANCELADO
				, IF(R.COLOR IS NULL,'#F31A83',R.COLOR) AS COLOR  
				FROM ADMIN_VIAJES V
				LEFT JOIN ADMIN_RAZONES_CANCELACION R ON V.ID_RAZON_CANCELACION = R.ID_RAZON_CANCELACION
				INNER JOIN SRV_ESTATUS  E ON V.ID_SRV_ESTATUS = E.ID_ADMIN_ESTATUS
				INNER JOIN SRV_CLIENTES C ON V.ID_CLIENTE 	  = C.ID_CLIENTE
				WHERE V.ID_SRV_ESTATUS IN (4,8)
				  AND V.FECHA_VIAJE BETWEEN '".$aFilter['FECHA_IN']."' AND '".$aFilter['FECHA_FIN']."'				
				GROUP BY V.ID_RAZON_CANCELACION
    ...
    public function getDetailsCancel($aFilter){
      	$this->query("SET NAMES utf8",false);     	
		$result= Array();
		
    	$sql ="SELECT *, E.ESTATUS AS N_ESTATUS, CONCAT(C.`NOMBRE`,' ',C.APATERNO,' ',C.AMATERNO) AS N_CLIENTE,  IF(R.DESCRIPCION IS NULL,'Sin Razon',R.DESCRIPCION) AS N_CANCELADO 
			FROM ADMIN_VIAJES V
			LEFT JOIN ADMIN_RAZONES_CANCELACION R ON V.ID_RAZON_CANCELACION = R.ID_RAZON_CANCELACION
			INNER JOIN SRV_ESTATUS  E ON V.ID_SRV_ESTATUS = E.ID_ADMIN_ESTATUS
			INNER JOIN SRV_CLIENTES C ON V.ID_CLIENTE 	  = C.ID_CLIENTE
